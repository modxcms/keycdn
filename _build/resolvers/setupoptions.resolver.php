    ...
    case xPDOTransport::ACTION_INSTALL:
        ...
        foreach ($settings as $key) {
            if (isset($options[$key])) {
                $setting = $object->xpdo->getObject('modSystemSetting',array('key' => 'kcdn.'.$key));
                if ($setting != null) {
                    $setting->set('value',$options[$key]);
                    $setting->save();
                } else {
                    $object->xpdo->log(xPDO::LOG_LEVEL_ERROR,'[KeyCDN] '.$key.' setting could not be found, so the setting could not be changed.');
                }
            }
        }
        ...
        if ($options['rules'] == 1) {
            ...
            foreach ($rules as $rule) {
                $obj = $object->xpdo->newObject('kcdnRule', $rule);
                $obj->save();
            }
        }
